...
- **Zustand split:** `selectedListStore` owns data (profiles, order, sort); `rosterUiStore` owns UI chrome (pinned, sort dropdown visibility). Keeps persist payload small and UI ephemeral.
- **Deck interaction:** Pointer drag vs click disambiguated with movement threshold; expanded card is a separate overlay so navigation and list actions stay accessible.
- **DnD portals:** `@hello-pangea/dnd` clones can disappear inside `overflow: hidden` glass containers — portaling the dragging row to `document.body` fixes it without changing library.
- **Performance:** Profile JSON already code-split per file via Vite glob imports; main bundle size is mostly React + motion + deck logic — lazy routes would be the next win.
- **Data coverage:** All 30 search profiles have detail JSON; loader falls back to search summary if a file is missing.
